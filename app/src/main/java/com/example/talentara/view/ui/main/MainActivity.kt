...
class MainActivity : AppCompatActivity() {
    ...
    private fun getUserDetailObserver(controller: NavController) {
        profileViewModel.getUserDetail.observe(this) { result ->
            when (result) {
                is Results.Loading -> {}
                is Results.Success -> {
                    val user = result.data.userDetail?.isOnProject
                    user?.let { isOnProject ->
                        setupActionButton(controller, isOnProject)
                    }
                }
                is Results.Error -> {}
            }
    ...
    private fun setupActionButton(controller: NavController, user: Int) {
