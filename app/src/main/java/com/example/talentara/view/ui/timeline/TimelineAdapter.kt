...
class TimelineAdapter(
    ...
    inner class ViewHolder(private val binding: TimelineItemBinding) :
        ...
        fun bind(item: TimelineProjectItem) {
            val formattedDeadline = formatDeadline(
            ...
            binding.apply {
                ...
                if (item.completedDate != null) {
                    ...
                    tvComplete.text =
                        itemView.context.getString(R.string.completed_in_d_days, completedDays)
                } else {
                    binding.tvRemaining.visibility = ViewGroup.VISIBLE
                    binding.tvComplete.visibility = ViewGroup.GONE
                    tvRemaining.text =
                        itemView.context.getString(R.string.remaining_days, daysRemaining)
                }
                tvEvidance.text =
                    itemView.context.getString(R.string.evidence_s, item.evidance ?: "-")
            ...
            binding.cvManagerApproved.apply {
                ...
                setCardBackgroundColor(
                    if (item.leaderApproved == 1) context.getColor(R.color.blue)
                    else context.getColor(R.color.dark_gray)
                )
                setOnClickListener {
                    if (isManager && item.leaderApproved == 0) {
                        onManagerApproveClick?.invoke(item)
                    }
                }
            }
            binding.cvClientApproved.apply {
                ...
                setCardBackgroundColor(
                    if (item.clientApproved == 1) context.getColor(R.color.blue)
                    else context.getColor(R.color.dark_gray)
                )
                setOnClickListener {
                    if (isClient && item.clientApproved == 0) {
                        onClientApproveClick?.invoke(item)
                    }
                }
            }
